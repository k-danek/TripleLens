...
cc_array       = np.zeros(6*length, np.double)
ca_array       = np.zeros(6*length, np.double)
...
for cluster_index in range(0, clustered_boundaries):
...
plt.rcParams.update({'font.size': 6, 'font.family': 'font.arial', 'svg.fonttype': 'none'})
