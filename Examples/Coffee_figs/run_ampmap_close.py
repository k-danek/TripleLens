...
for i in range(0, lc_steps):
  ...
  lc_irs.get_lc_irs(pos_ini_x,row_y,pos_fin_x,row_y)
...
axs[1].plot([lc_ini_x, lc_fin_x], [lc_ini_y, lc_fin_y], color="green", linewidth=2)
...
# Add colorbar with correct limits
...
pos_ini_x_rounded = float(np.round(pos_ini_x+0.05, 1))
pos_ini_y_rounded = float(np.round(pos_ini_y+0.1, 1))
...
fig.savefig('amplification_map_close.svg')
fig.savefig('amplification_map_close.pdf')
fig.savefig('amplification_map_close.eps')
