...
for cluster_index in range(0,5):
  filename = "clustered_boundaries_"+str(source_index)+"_"+str(cluster_index)
  with open(filename, newline='') as csvfile:
    # Adding spurious elements because of inferior knowledge of Python.
    clustered_boundaries.append([LineSegment(0,0,0)])
...
ax1.scatter(par_manager.z1_x, par_manager.z1_y, marker='+', color='black', s=75)
ax1.scatter(par_manager.z2_x, par_manager.z2_y, marker='+', color='black', s=75)
ax1.scatter(par_manager.z3_x, par_manager.z3_y, marker='+', color='black', s=75)
...
clustered_boundaries[0] = clustered_boundaries[0] + clustered_boundaries[1]

##axis_list.append(plt.subplot(subplot_list[i]))
axis_list.append(plt.subplot2grid((2,4), (1,2)))
for tick in axis_list[0].xaxis.get_majorticklabels():
  tick.set_verticalalignment("top")
bounding_box = Get_bounding_box_segments(clustered_boundaries[0], 1.15)
for segment in clustered_boundaries[0]:
  axis_list[0].plot([segment.nL,segment.nR],[segment.nY,segment.nY], color='green')
axis_list[0].scatter(cc_real, cc_imag, color='blue', s=0.02)
axis_list[0].set_aspect('equal')
axis_list[0].set_xlim([bounding_box[0], bounding_box[1]])
axis_list[0].set_ylim([bounding_box[2], bounding_box[3]])
for tick in axis_list[0].xaxis.get_majorticklabels():
  tick.set_transform(tick.get_transform() + x_tic_offset)
for tick in axis_list[0].yaxis.get_majorticklabels():
  tick.set_transform(tick.get_transform() + y_tic_offset)
